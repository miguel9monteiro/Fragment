...
export const metadata: Metadata = {
  title: "Contribute",
  description:
    "How to contribute pitches, sessions, macro outlooks, quant presentations, and glossary terms to the PMC platform.",
};
...
const CONTENT_TYPES = [
  {
    icon: FileText,
    label: "Stock pitch",
    title: "An investment pitch teaching page",
  ...
  },
  {
    ...
    title: "A learning session",
    location: "/content/sessions/<slug>.mdx",
    body: "A focused lesson on one concept or method. Use Callouts, SelfChecks, DeepDives, FormulaBlock. 8–15 minutes of read time.",
  },
  {
    icon: Globe,
    label: "Macro outlook",
    title: "A macro outlook",
    location: "/content/macro/<slug>.mdx",
    body: "A periodic reading of the macro backdrop — rates, growth, liquidity. Tagged by period and (optionally) region.",
  },
  {
    icon: LineChart,
    label: "Quant presentation",
    title: "A quant presentation",
    location: "/content/quant/<slug>.mdx",
    body: "A quantitative piece — factor work, backtesting, ML, options. Same shape as a session, with quant-specific tags.",
  },
  {
    icon: Hash,
    label: "Glossary term",
    ...
    body: "A short, precise definition of a term used across the library. Two sentences max. Link related items where the concept is explored.",
  },
] as const;
    ...
    <>
      <section className="container pt-12 pb-10 border-b border-border">
        ...
        <p className="mt-4 text-muted-foreground max-w-2xl leading-relaxed">
          Every pitch, session, macro outlook, quant presentation, and glossary
          term lives as plain markdown in a public repository. To add or improve
          content, fork the repo, write your MDX, and open a pull request — two
          senior members review and merge.
        </p>
      ...
      <section id="how-to" className="container py-16 border-t border-border">
        ...
        <p className="text-muted-foreground max-w-2xl leading-relaxed mb-10">
          Pitches, sessions, macro outlooks, and quant presentations are the
          four content categories the club publishes. Glossary entries support
          all four.
        </p>

        <div className="grid gap-px bg-border border border-border md:grid-cols-2 lg:grid-cols-3">
          {CONTENT_TYPES.map((c) => (
            <ContentType key={c.label} {...c} />
          ))}
        </div>
      </section>
      ...
      <section className="container py-16 border-t border-border">
        ...
        <p className="text-muted-foreground max-w-2xl leading-relaxed mb-10">
          Every page can use these components inside MDX. Full usage examples
          live in <code className="text-xs">CONTRIBUTING.md</code>.
        </p>

        <div className="grid gap-px bg-border border border-border md:grid-cols-2 lg:grid-cols-3">
          {COMPONENTS.map((c) => (
            <div key={c.name} className="bg-card p-5">
              <p className="text-xs text-steel mb-2">{c.name}</p>
              <p className="text-base font-semibold mb-1">{c.title}</p>
        ...
        <div className="grid gap-12 md:grid-cols-2 items-start">
          <div>
            ...
            <p className="text-muted-foreground leading-relaxed">
              ...
              accuracy, clarity, correct use of the MDX components, and
          ...
          <div className="border border-border p-6 rounded-sm bg-secondary/40">
            ...
            <ul className="space-y-3 text-sm leading-relaxed">
              <li className="flex gap-3">
                <span className="text-[10px] text-steel tnum mt-1.5">01</span>
                Numbers tie out — every figure has a source or a calculation.
              </li>
              <li className="flex gap-3">
                <span className="text-[10px] text-steel tnum mt-1.5">02</span>
                Concepts are introduced before they're used.
              </li>
              <li className="flex gap-3">
                <span className="text-[10px] text-steel tnum mt-1.5">03</span>
                Self-checks test understanding, not memorization.
              </li>
              <li className="flex gap-3">
                <span className="text-[10px] text-steel tnum mt-1.5">04</span>
...
function Step({
  ...
  return (
    <div className="bg-card p-7 flex flex-col gap-3">
      <p className="text-xs text-steel tnum">{number}</p>
...
function ContentType({
  ...
  return (
    <div className="bg-card p-7 flex flex-col gap-3">
      ...
      <p className="text-xl font-semibold tracking-tight">{title}</p>
      <p className="text-sm text-muted-foreground leading-relaxed">{body}</p>
      <p className="text-[11px] text-foreground/70 mt-auto pt-3 border-t border-border/70 break-all">
        {location}
      </p>
    </div>
  );
